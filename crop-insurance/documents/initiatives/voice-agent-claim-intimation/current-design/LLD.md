...
| `claim_intimations` | `id`, transaction number, FIN, status, source channel, selected land/policy, event facts, deadline, rule version, confirmation time |
| `claim_evidence` | `id`, intimation ID, object URI, checksum, type, capture time, metadata |
...
```text
Voice session:
STARTED -> CONSENTED -> IDENTIFIED -> LAND_SELECTED -> DETAILS_CAPTURED
-> SUMMARY_READ -> CONFIRMED -> COMPLETED
                         \-> HUMAN_HANDOFF | ABANDONED

Claim intimation:
DRAFT -> TRIAGED -> EVIDENCE_PENDING | READY_FOR_CONFIRMATION
READY_FOR_CONFIRMATION -> CONFIRMED -> QUEUED -> SUBMITTED
...
buildPacket(canonicalIntimation) -> insurer payload + attachments
...
Week 1-2: schema, FIN/land migration, consent, canonical draft API, pilot rule seed.  
Week 3-4: call simulation, Whisper + Qwen tool loop, land disambiguation, read-back.  
Week 5-6: Kafka outbox/workers, notification, document-knowledge review flow.  
Week 7-8: insurer export adapter, operator queue, test calls, security and failure tests.  
Week 9-12: approved live telephony/WhatsApp pilot, measured rollout, and integration refinement.
