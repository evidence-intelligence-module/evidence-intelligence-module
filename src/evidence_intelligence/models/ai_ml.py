...
class AiMlModel:
    def __init__(self):
        ...
        self._label_provenance: str | None = None
    ...
    def save(self, path: str | Path, *, label_provenance: str) -> None:
        """Persists the trained model, its disclosed validation metrics, and a
        declaration of where its training labels came from.

        Raises if called on an untrained instance — there's nothing honest to
        disclose from an untrained model, so it shouldn't be saved as if it
        were a real artifact.

        `label_provenance` is mandatory and unvalidated, and both of those are
        deliberate. Sourcing training data is out of scope for this module
        (constitution.md §9.2), so it cannot verify where a supplier's labels
        came from — but a §65B package that reports a calibrated accuracy
        figure while saying nothing at all about the data behind it is worse
        than one that reports what it was told. A declaration this module
        cannot check is still a declaration somebody made.

        This is also what remains of the Constitution §4 gate that a
        claim-outcome endpoint would have enforced at ingress: with no ingress,
        the honest substitute is to record and surface the claim rather than to
        police it. State the source plainly, e.g. "insurer pilot survey,
        Maharashtra 2025-26, non-CCE" or "unknown — supplied without
        provenance"."""
        if not self._is_trained:
            raise RuntimeError("cannot save an untrained AiMlModel — call fit() first")
        if not label_provenance or not label_provenance.strip():
            raise ValueError(
                "label_provenance is required — state where the training labels came from, "
                "or 'unknown' if the supplier did not say. It is reported in every evidence "
                "package generated by this model (constitution.md §9.2)."
            )
        ...
        joblib.dump(
            {
                ...
                "label_provenance": label_provenance.strip(),
            },
            path,
        )
    ...
    def load(cls, path: str | Path) -> AiMlModel:
        ...
        # Artifacts saved before label_provenance was mandatory carry no such
        # key. They load, and report the absence rather than an empty string —
        # "not recorded" is a true statement about the artifact, where "" would
        # read as a provenance that was declared and left blank.
        instance._label_provenance = payload.get("label_provenance") or "not recorded"
    ...
    def predict(self, feature_vector: dict[str, float], harvest_index: float) -> AiMlResult:
        ...
        if self._is_trained:
            ...
            accuracy = {
                ...
                # Reported as declared by whoever supplied the training data,
                # never verified — this module does not source labels
                # (constitution.md §9.2). Present only on the trained path:
                # an untrained placeholder has no training data to have a
                # provenance for.
                "label_provenance": self._label_provenance or "not recorded",
            }
        else:
