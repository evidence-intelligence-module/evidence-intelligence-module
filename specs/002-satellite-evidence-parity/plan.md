...
**Scale/Scope**: Commercial (paid/tasked) satellite source budget and per-region/per-season volume thresholds are **not specified in any source document** and are deliberately not invented here, consistent with `CLAUDE.md`'s no-invented-figures convention — flagged as an open item in [research.md](./research.md) and tracked in [`issue/open query - commercial satellite tasking budget and volume thresholds.md`](./issue/open%20query%20-%20commercial%20satellite%20tasking%20budget%20and%20volume%20thresholds.md), the same treatment `001` gave its own open request-volume question.
...
│   ├── commercial_tasking_client.py  # NEW — isolated, budget-gated client for tasked VHR optical / commercial SAR (FR-002, FR-013)
...
├── packaging/
│   └── report_generator.py           # MODIFIED — surfaces confidence tier, source provenance, discrepancy flags, non-equivalence statement
└── store/
    └── schema.py                      # MODIFIED — new columns/tables per data-model.md

src/tests/
├── contract/              # New: validates api/ against contracts/ extension
├── integration/           # New: end-to-end per spec.md User Stories 1-4
└── unit/                  # New: source_registry, confidence_tier, foundation_features, crop_calendar_crosscheck
```
