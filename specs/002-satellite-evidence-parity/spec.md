...
- What happens when a request would benefit from an enhanced source (higher-resolution optical or sovereign SAR) that is unavailable for that field, date, or region? The module MUST fall back to the baseline sources and MUST record that the enhanced source was considered but not used, rather than silently omitting the consideration. *(Reworded 2026-08-13: this case was previously about paid/tasked commercial imagery and a budget gate; commercial procurement is out of scope per `constitution.md` §9.2, but the record-the-decision requirement survives for free enhanced sources.)*
...
- **FR-002**: The module MUST support ingestion of higher-resolution (sub-10m) optical and/or SAR sources **from free, open, or sovereign providers** as an additional evidence tier for requests where the existing 10–30m baseline is structurally insufficient (small field size or monsoon-season cloud cover), where such a source is available and in scope. *(Narrowed 2026-08-13: commercial sources removed — procurement is out of scope per `constitution.md` §9.2.)*
...
- **FR-004**: The module MUST compute and report a small number of plain-language confidence tiers per evidence package, derived from and traceable to a per-request record of which evidence inputs were actually available and used — of which the existing per-component and ensemble confidence figures (`documents/modeling-approach.md` §5, §7) are one input among several. *(Reworded 2026-08-13.)* The original wording tied the tier to the confidence figures alone; that proved too narrow in two ways. Practically, the ensemble figure evaluates to a constant in the shipped configuration, so a tier derived from it alone would assign every package the same label. Substantively, what a reviewer needs to know is whether the *evidence* is strong — was the field visible through cloud, was a historical baseline available, did SAR substitute for optical, how many declared features were measured rather than defaulted — and two packages can carry identical confidence figures with opposite answers to those questions. See [`issue/open query - confidence tier threshold values (FR-004).md`](./issue/open%20query%20-%20confidence%20tier%20threshold%20values%20%28FR-004%29.md) for the provisional rule table adopted, and `tasks.md` `T0-09` for the record it reads from.
...
- **FR-006**: ~~The module MUST support an optional interface for attaching supplementary evidence (e.g., a geotagged photo) to a package.~~ **Removed 2026-08-13 — out of scope per `constitution.md` §9.1 (data minimisation).** A geotagged photograph is personal data about an identifiable individual, arriving from outside, into a store with a ten-year retention floor; the request surface does not widen. Identifier retained rather than renumbered so existing cross-references resolve.
...
- **FR-009**: Every evidence package MUST document which satellite sources were used to produce it, including source name, resolution, and acquisition date, per Constitution Principle I/II.
...
- **FR-013**: The module MUST record, per evidence package, which enhanced sources were considered and whether each was used, as part of the package's provenance trail. *(Narrowed 2026-08-13: the commercial tier is gone, but "an enhanced free source was considered and not used" is still provenance a reviewer needs — recording the decision, not only the outcome.)*
- **FR-014**: ~~The module SHOULD support human-in-the-loop, segmentation-model-assisted refinement of a submitted field or damage-extent polygon.~~ **Removed 2026-08-13 — out of scope per `constitution.md` §9.2.** Geometry arrives with the request; deriving or correcting a field boundary is upstream supply. Identifier retained rather than renumbered so existing cross-references resolve.
...
- **FR-019**: Commercial satellite procurement — vendor contracts, tasking budgets, licence negotiation, and any paid VHR optical or commercial SAR source — is **out of scope per `constitution.md` §9.2**, permanently. *(Rewritten 2026-08-13: this previously read "not authorized for this rollout", i.e. a deferral. It is now a boundary, and crossing it requires a Constitution §8 amendment rather than a budget decision.)* Enhanced-tier sourcing is limited to free, open, and sovereign sources — the existing baseline plus ISRO sources via a dedicated access client, since those are not available through the existing GEE integration.

The following five were added on 2026-08-13. FR-020–FR-022 close capture-quality factors this spec's own Input paragraph names — cross-sensor harmonization, mixed-pixel effects on India's small field sizes, atmospheric/BRDF correction, SAR speckle/decorrelation — which had no requirement behind them; FR-023 closes a reproducibility gap in the thermal signal added by User Story 5.
...
- **FR-024**: ~~The module MUST support an optional, channel-agnostic interface for recording the outcome of the claim an evidence request supported, and MUST be able to export those outcomes as a labeled training/calibration set.~~ **Removed 2026-08-13 — out of scope per `constitution.md` §9.2 (training-label sourcing) and §9.1 (data minimisation).** Labeled data is supplied by an external party; the module does not generate, capture, or export it. Two consequences are recorded rather than hidden: the causation low-confidence threshold and the confidence-tier values both wanted claim-outcome data and now have no in-module source, so either the supplier contract includes outcomes or both thresholds stay permanently unset under the existing "unset rather than invented" discipline. Identifier retained rather than renumbered so existing cross-references resolve.
...
- **Satellite Source Registry**: The catalog of satellite sources available to the module — baseline (Sentinel-1/2, Landsat, MODIS) and enhanced (higher-resolution optical and sovereign ISRO sources) — each with resolution, revisit cadence, and sensor type, used to select sources per request. *(Narrowed 2026-08-13: the commercial tier and the free-vs-commercial access model are gone with `constitution.md` §9.2; every registered source is free or sovereign.)*
...
- ~~**Supplementary Evidence Attachment**~~ — removed 2026-08-13 with FR-006, per `constitution.md` §9.1.
- ~~**Claim Outcome**~~ — removed 2026-08-13 with FR-024, per `constitution.md` §9.2. Labeled data arrives from an external supplier; the module does not capture it.
