...
- **FR-024** *(added 2026-08-13)*: The module MUST support an optional, channel-agnostic interface for recording the outcome of the claim an evidence request supported, and MUST be able to export those outcomes as a labeled training/calibration set. Recording an outcome MUST NOT re-run analysis, alter any issued package, or change a confidence tier — observation of what happened must not retroactively shape what the module said would happen. Without this the module discards the only labels its own operation produces, and no accuracy or threshold claim in this specification is ever verifiable; it is therefore a prerequisite for the Pilot & Validation phase (`documents/README.md` §8) rather than an output of it. Every recorded loss figure MUST declare where it was assessed, and the module MUST reject any outcome whose declared assessment source is not authorised for that deployment. This requirement does not presume any answer to whether CCE-derived outcomes may be supplied through it — that remains the open question in `specs/001-evidence-generation-pipeline/issue/` — but because this interface is built before that question is settled, the boundary must be enforced by configuration rather than left to callers' discretion (Constitution Principle IV).
...
- **Claim Outcome** *(added 2026-08-13)*: What happened to the claim an evidence request supported — a small closed outcome category plus, where the caller has one, an independently assessed loss fraction. Recorded after the fact, never used to alter an issued package, and the source of the labels every accuracy and threshold claim in this specification depends on.
