...
- [ ] T0D-02 [P] **Update both `plan.md` Project Structure trees** — [`001`](../001-evidence-generation-pipeline/plan.md) lines ~64-89 and [`002`](./plan.md) lines ~71-105 — to list `observation.py`, `models/registry.py`, and `packaging/assembly.py`. Not staleness: `002/plan.md` was edited on 2026-08-13 to add Phase 0.6's `validation/` and `store/label_export.py`, so the omission is a specific miss. `001/plan.md` additionally claims "HLD §3's component table maps directly onto the package layout above, one subpackage per component", which becomes false once Phase 0.4 lands and must be reworded together with `T0D-01`
