...
- **`T0D-NN`**: `documents/` reconciliations surfaced by Phase 0.4 (`D` for documents), added 2026-08-13. Separate because these change hand-authored domain docs rather than code, and `documents/hld.md` states it is the source of truth for architecture — so code landing first makes these *required*, not optional.
...
**Update (2026-08-13): commercial satellite procurement is out of scope, not merely unbudgeted.** `constitution.md` §9.2 makes vendor contracts, tasking budgets, and licence negotiation a permanent boundary; crossing it needs a §8 amendment, not a budget decision. `T037` is struck and `COMMERCIAL_TASKING_ENABLED` is gone — the disabled stub was a mechanism waiting for a decision that has now gone the other way. The enhanced tier for User Story 1 runs on Sentinel-1 SAR (existing) plus ISRO's Resourcesat-2A LISS-4/EOS-04 via a new Bhoonidhi client (T009). PlanetScope is commercial and therefore out (see `research.md` §2's correction note, which remains accurate as history).
...
- [ ] T0-06 Compute the causation engine's temporal and spatial terms from observed data (break-point date of the index time series; actual distance from the geometry to the weather anomaly) instead of passing hardcoded `1` and `0.0` at the call site, in `src/evidence_intelligence/pipeline.py` and `src/evidence_intelligence/causation/scoring.py`
  - **Also in scope (added 2026-08-13):** the magnitude term takes `abs(anomaly_score or 0.0)`, turning an uncomputable precipitation anomaly into a measured zero. `_magnitude_correlation(0.0, d)` = `100·(1−d)`, so with a large observed NDVI drop (`d`=0.9) the term scores **10 out of 100** — a 22-point swing on the 100-point causation confidence printed in every §65B package — and a *bigger* observed loss with *unknown* weather scores *lower*, indistinguishable from a genuinely measured "no anomaly occurred". Needs a `None` path that redistributes the 25 points across measured terms rather than scoring zero; that changes the score's structure, which is why it belongs with this task. `observation.py` already records the substitution in the signal's provenance (`T0R-02`). Note there is **no external authority to consult**: `causation`, `causal`, and `attribution` appear zero times in `yestech_manual_2023.md`'s 6,565 lines — YES-TECH does yield estimation, not causal attribution, so `evidence-flow-spec.md` §5 is the only source and this is a pure design call.
  - **Partially done 2026-08-13.** The fabrication half is fixed: every term is now `float | None`, unmeasurable terms are excluded rather than scored, the weighted score renormalizes over measured terms, and the package discloses which of the four contributed and why the rest did not. `days_between=1` and `distance_km=0.0` now pass as `None`; an unmeasured NDVI drop passes as `None` instead of `0.0`; `phenology_checked=False` distinguishes "no imagery to check" from "checked, nothing wrong". `causation_confidence_score` is nullable in the schema. Measured effect on the golden fixtures: a request with no imagery went **98 -> null**, and a full optical pair went 76 -> 46. **Still open here**: computing the temporal term needs a break-point date (`T05-05`) and the spatial term needs the anomaly's footprint, which nothing computes — that half is untouched.
  - **Residual, needs a decision (2026-08-13).** Renormalizing over measured terms puts a one-term score on the same 0-100 scale as a four-term score, so `flood_sar_substitution` now reports **90 from `physiological_plausibility` alone** — above `healthy_hailstorm`'s 46 from two terms. Fewer measured terms still buys a higher number, just less extremely than before. Fixing it means choosing a minimum term count (or a coverage penalty) before a score is reportable, which is an **unsourced cut point** of the same class as the damage bands (`T0-17`) and the confidence tier — so it is deliberately not invented here. Track with [`001`'s causation threshold query](../001-evidence-generation-pipeline/issue/open%20query%20-%20causation%20confidence%20low-confidence%20threshold%20%28FR-024%29.md), which owns the same figure's other half.
...
- [ ] T0R-09 **Re-scope the nine `002` tasks that target code Phase 0.4 supersedes** (found by the 2026-08-13 cross-check; see [`pipeline-decomposition-design.md`](./pipeline-decomposition-design.md) §10a). `T012`, `T017`, `T025`, `T029`, `T036`, `T043` each say "surface `<field>` … in `packaging/report_generator.py`", which `T0R-06` replaces as the assembly point — implementing them literally recreates the two-divergent-assembly-paths problem `T0R-06` exists to close, just with `002`'s fields instead of the preliminary tier's. `T024`, `T028`, `T035` write to `pipeline.py` directly, bypassing `T0R-07`'s `_persist`, which is designed to be the only caller of `store.add_*`. Rewrite all nine against `packaging/assembly.py` and `PipelineRecord`. Also fix two design defects §10a records: `PipelineRecord.satellite_row` must be `satellite_rows: list[...]` for `T0-08`'s one-row-per-source model, and boundary rule 2 must be scoped to the six pipeline stages rather than the whole codebase, since `api/routes.py` legitimately imports `EvidenceStore`
...
- [ ] T0D-01 **Add the observation stage and the packaging split to [`documents/hld.md`](../../documents/hld.md) §3.** The Component Breakdown table maps ten components 1:1 onto the module layout and has no row for anything resembling `observation.py`, which Phase 0.4 makes a first-class stage that no downstream stage may bypass — a new architectural concept, not an internal refactor. §3 also names one "Report/Package Generator" where `T0R-06` splits assembly (`packaging/assembly.py`) from rendering (`report_generator.py`). `hld.md` §1 states it is "the source of truth for the architecture" and that `specs/` may lag it, so the direction of truth runs hld → specs: landing the code without this leaves the domain doc describing an architecture that no longer exists
- [ ] T0D-02 [P] **Update both `plan.md` Project Structure trees** — [`001`](../001-evidence-generation-pipeline/plan.md) lines ~64-89 and [`002`](./plan.md) lines ~71-105 — to list `observation.py`, `models/registry.py`, and `packaging/assembly.py`. Not staleness: `002/plan.md` was edited on 2026-08-13 to add Phase 0.6's `validation/`, so the omission is a specific miss. *(`store/label_export.py` was added in that same edit and removed later the same day with `TV-02` — `constitution.md` §9.2.)* `001/plan.md` additionally claims "HLD §3's component table maps directly onto the package layout above, one subpackage per component", which becomes false once Phase 0.4 lands and must be reworded together with `T0D-01`
- [ ] T05-11 [P] **Stop persisting an unmeasured precipitation reading as a measured `0.0`.** `schema.py`'s `WeatherCorrelationResult.observed_value` is non-nullable while its neighbours `historical_baseline` and `anomaly_score` are `float | None`, which forces `pipeline.py`'s `observed_value=weather.precipitation.observed_value or 0.0`. `_window_mean` returns `None` when CHIRPS has no computable statistic for the window, so a real "not measured" is collapsed into "0.0 mm fell" and kept for the 10-year retention period. Direction of error is peril-dependent: it spuriously corroborates a drought claim and spuriously rebuts an excess-rainfall one. Does **not** reach the printed package today — `report_generator.py` never reads it — but it is in the record a §65B discovery request would produce. Make the column nullable and drop the `or 0.0`
- [ ] T05-12 [P] **Add `ORDER BY` to the store's list queries.** `evidence_store.py`'s `list_component_results` and `list_weather_results` have no ordering, and `api/routes.py` takes `weather_results[-1]` to pick the causation score shown alongside the latest package. SQL result order is not guaranteed without `ORDER BY`, so on a request with more than one pipeline run — exactly FR-022's weather-only-preliminary-then-complete case — the displayed score may belong to a superseded package. A provenance defect rather than a fabricated figure, but it undermines the same §65B chain-of-custody argument
...
- [x] T05-08 **Done 2026-08-13.** `gee_client.optical_composite` now selects SWIR (`B11` for Sentinel-2, `SR_B6` for Landsat — `B12` does not overlap MODIS B6's 1628-1652nm and is the wrong band) and computes `LSWI = (NIR-SWIR)/(NIR+SWIR)` over the same window and per-pixel mask as NDVI. `ImageryComposite.lswi_value` carries it; `observation.py` feeds Component 1's water-stress scalar a real measurement and populates `lswi_deviation`, a `modeling-approach.md` §6 DSI indicator that had never been computed. Where a source carries no usable SWIR band, NDVI still stands in but the provenance says so, and `lswi_deviation` stays **absent** rather than becoming an NDVI deviation wearing an LSWI name. Measured effect: Component 1's damage fraction moved 0.8011 -> 0.7749 on the golden fixtures. Suite 153 -> 155.
  - **`lswi_deviation` does not yet affect the DSI**: it has no historical archive, so entropy weighting gives it zero weight. `T05-10` must build an archive for LSWI as well as NDVI or this indicator stays inert.
  - Harness change made here: `component_results` is keyed by component name rather than list position, because an index-based `known_wrong` path shifts whenever Component 3 is enabled and is unreadable in a review.
- [ ] T05-09 [P] **Fetch pre- and post-event temperature separately.** `pipeline.py` passes one `observed_temp_c` as *both* `pre_event_temp_c` and `post_event_temp_c`, so `_temperature_stress_scalar` returns the same value on both sides and cancels exactly out of `(expected − observed)/expected`. Verified: damage fraction is bit-identical at 12/25/38 °C. The same holds for insolation, which is a constant proxy. Component 1 is therefore a closed-form function of pre/post NDVI alone — while contributing `calibration_confidence = 0.85` to a confidence-weighted ensemble that reads agreement as corroboration, which is exactly the hazard `models/csm_assimilation.py`'s docstring warns about for Component 3. ERA5-Land is already ingested; only the pre-event window call is missing. Cheapest change that makes Component 1 stop being purely NDVI-driven. See [`issue/open query - RUE model applied to part of a season`](./issue/open%20query%20-%20RUE%20model%20applied%20to%20part%20of%20a%20season%20%28modeling-approach.md%202%29.md) for the larger question this sits under, and cap or disclose the 0.85 in the meantime
- [ ] T05-10 **Build a historical *deviation* archive for the DSI.** `pipeline.py` passes `historical_ndvi` — absolute NDVI index values — as the archive for `ndvi_deviation`, which is a drop. `modeling-approach.md` §6 specifies normalizing the deviation "using the field's own historical archive as the reference range", so the archive must hold deviations. Consequence, pinned by `T0R-01`'s `varied_historical_archive` fixture: a 0.45 drop normalized against a 0.62–0.81 index range clips below the floor, so the package reports **DSI 0.0 for a field that lost 0.45 NDVI** — and it errs against the claimant. Needs two windows per prior season at the same calendar offsets, a small extension of `historical_composite`'s existing per-year call; sequence with `T05-03` (robust baselines) and `T05-05` (per-field time series), which restructure the same code. Until it lands, `T0R-05` excludes the indicator rather than normalizing against the wrong quantity
...
## Phase 0.6: Label-Free Validation (added 2026-08-13, narrowed 2026-08-13)

**Purpose**: measure the things that *can* be measured without ground truth.

**Scope change 2026-08-13.** This phase originally opened with label capture — `TV-01`/`TV-01a`/`TV-02`, an outcome table and endpoint that would turn the module's own operation into training labels. Training-label sourcing is now out of scope per `constitution.md` §9.2: labeled data arrives from an external supplier, and the module neither generates nor exports it. Those three tasks are struck below, along with `TV-05`, whose stated justification was label scarcity. What remains needs no labels and never did.

- ~~TV-01 Record claim outcomes (`claim_outcomes` table + `POST /evidence-requests/{request_id}/outcome`)~~ — **struck**, with FR-024. Also barred by §9.1: the request surface does not widen
- ~~TV-01a Constitution §4 gate on the outcome endpoint~~ — **struck** with `TV-01`. It gated an ingress that no longer exists. Its purpose survives in a different form: `label_provenance` on the saved model artifact, which records what a supplier *declared* about their labels' origin, since the module cannot verify it
- ~~TV-02 Export a training set from captured outcomes~~ — **struck** with `TV-01`. There are no captured outcomes to export
- ~~TV-05 Ablation harness~~ — **struck.** It was explicitly "the honest, available version of what User Story 3's independent test is trying to do while `D2` is open". With labeled data supplied externally, the direct measurement — MAE/RMSE/NRMSE on a held-out split — replaces it

**Consequence recorded rather than hidden**: the causation low-confidence threshold (`001` FR-024) and the confidence-tier values both wanted claim-outcome data. Neither now has an in-module source. Either the supplier contract includes outcomes, or both stay permanently unset under the existing "unset rather than invented" discipline — packages ship without a low-confidence label rather than with a fabricated one.
...
**Checkpoint**: the module's false-positive rate and reproducibility are measurable today, without ground truth and without waiting for the pilot.
...
- [ ] T002 [P] Create module skeletons with `__init__.py` exports: `src/evidence_intelligence/ingestion/source_registry.py`, `src/evidence_intelligence/ingestion/bhoonidhi_client.py`, `src/evidence_intelligence/ingestion/crop_calendar_crosscheck.py`, `src/evidence_intelligence/models/foundation_features.py`, `src/evidence_intelligence/models/confidence_tier.py`, `src/evidence_intelligence/models/thermal_stress.py`, `src/evidence_intelligence/models/red_edge_indices.py` — *(2026-08-13: `commercial_tasking_client.py` and `field_boundary_assist.py` removed from this list, out of scope per `constitution.md` §9.2)*
- [ ] T003 [P] Add new environment/config entries (`PRESTO_MODEL_PATH`, `WORLDCEREAL_ENDPOINT`, `BHOONIDHI_API_KEY`, `ECOSTRESS_AWS_REGION`) in `src/evidence_intelligence/config.py` — *(2026-08-13: `COMMERCIAL_TASKING_ENABLED` removed with the commercial tier; no paid-provider credentials exist to configure)*
...
- [ ] T004 Extend database schema per `data-model.md`: new columns on `satellite_analysis_results` (`source_class`, `considered_not_used`), `evidence_packages` (`confidence_tier`, `confidence_tier_guidance`, `cce_non_equivalence_statement`), and `model_component_results` (`red_edge_index_type`, `red_edge_index_value`); new tables `foundation_model_feature_sets`, `crop_calendar_cross_checks`, `thermal_stress_signals` — as SQLAlchemy models + Alembic migration in `src/evidence_intelligence/store/schema.py`. **Fold in `T0-08` (per-source satellite rows) and `T0-16` (package lineage) so the schema migrates once.** Note the `data-model.md` changes made 2026-08-13 after this task was written: `crop_calendar_cross_checks` carries a three-state `outcome` enum rather than a boolean `discrepancy_flag` (plus `reference_accuracy`/`pure_pixel_count`); `thermal_stress_signals` carries `overpass_local_solar_time`/`baseline_overpass_window`; and `access_model`, `supplementary_evidence_attachments` and `claim_outcomes` are all **gone** — out of scope per `constitution.md` §9, so this migration must not create them
...
- [ ] T011 [US1] Record `source_class`/`considered_not_used` provenance on every `SatelliteAnalysisResult` row, including sources evaluated but not used, in `src/evidence_intelligence/ingestion/imagery.py` (depends on T004, T010)
...
**Goal**: Classify every evidence package into a plain-language confidence tier (High/Medium/Low), derived from existing ensemble confidence, with an explicit non-equivalence-to-CCE statement for the lowest tier. *(2026-08-13: the supplementary-evidence attachment was removed from this story with FR-006, per `constitution.md` §9.1.)*
...
- ~~T014 Contract test for the supplementary-evidence endpoint~~ — **struck 2026-08-13** with FR-006 (`constitution.md` §9.1)
- [ ] T015 [P] [US2] Integration test for `quickstart.md` Scenario 2 (high/low confidence tiers, non-equivalence statement) in `src/tests/integration/test_confidence_tier_fallback.py`
...
- ~~T018 Implement the supplementary-evidence endpoint~~ — **struck 2026-08-13** with FR-006. The request surface does not widen (`constitution.md` §9.1); a geotagged photograph is personal data arriving from outside into a ten-year store. The earlier `caller_supplied_metadata` removal is now moot — the whole endpoint is gone
- ~~T019 Validation/error handling for the supplementary-evidence endpoint~~ — **struck 2026-08-13** with T018
...
- ~~T037 Implement commercial tasking client~~ — **struck 2026-08-13.** Commercial procurement is out of scope per `constitution.md` §9.2. The stub existed so a future budget decision would need only a config change; that decision has instead gone the other way, and keeping the mechanism would invite crossing a boundary without the §8 amendment it now requires
- ~~T038 Implement optional field/damage-boundary segmentation assist~~ — **struck 2026-08-13** with FR-014. Geometry arrives with the request; deriving or correcting it is upstream supply (`constitution.md` §9.2). The SAM2 remote-sensing domain-gap finding stays in `documents/research/satellite-parity-global-precedent-research.md` §4.2 as research
...
- [ ] T040 [P] Update `GUIDE.md` with operational instructions for the new capabilities (source registry configuration, Bhoonidhi setup, confidence-tier interpretation, foundation-model training, thermal/red-edge signals)
...
- Commercial tasking is out of scope per `constitution.md` §9.2 — `T037` struck, `COMMERCIAL_TASKING_ENABLED` removed. The earlier free-only decision in `issue/open query - commercial satellite tasking budget and volume thresholds.md` is superseded by a boundary rather than a budget
