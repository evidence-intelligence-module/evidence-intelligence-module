...
- [ ] T05-08 [P] **Compute real LSWI from SWIR.** `modeling-approach.md` §2 and `yestech_manual_2023.md` Appendix 1 both specify `LSWI = (NIR − SWIR)/(NIR + SWIR)` as Component 1's water-stress input; `observation.py` currently records NDVI standing in for it, because `gee_client.py` selects only `B8`/`B4` (Sentinel-2) and `SR_B5`/`SR_B4` (Landsat) — SWIR is never requested. `B11` and `SR_B6` are in the **same collections, behind the same per-pixel cloud masking**, so this is one additional `normalizedDifference` and no new data source, licence, or vendor. Makes the RUE chain's water-stress term a measurement instead of a stand-in, and gives the DSI's `lswi_deviation` indicator — named in `modeling-approach.md` §6 and never once populated — its first real input. Note `B11` is 20 m against NDVI's 10 m, so fix the reduction scale explicitly rather than letting GEE pick. In `src/evidence_intelligence/ingestion/gee_client.py`, `observation.py`, `pipeline.py`
