...
|---|---|---|---|
| **Open — split & narrowed** 2026-08-13 | [`open query - AI-ML training data source and CCE-label question.md`](./open%20query%20-%20AI-ML%20training%20data%20source%20and%20CCE-label%20question.md) | post-implementation review | `tasks.md` T017. Now scoped to **per-field damage-magnitude labels only**, and whether historical CCE outcomes may supply them. "Labels" turned out to be three datasets: magnitude (here), claim outcomes (work, not a decision → `002` `TV-01`/`TV-02`), and reference-product accuracy (→ `002`'s crop cross-check query). Still gates `002`'s User Story 3, SC-002, its mirror query, and the SAR magnitude question — but **no longer** SC-001, the causation threshold, or tier calibration, which want claim outcomes instead |
