...
Separately, [`specs/002-satellite-evidence-parity/tasks.md`](specs/002-satellite-evidence-parity/tasks.md) carries three pre-feature task groups that are **not** open questions and are blocked by none of the above: **Phase 0** base-pipeline corrections (defects with unambiguous right answers; several block the rest of `002` from being measurable at all), **Phase 0.5** evidence-processing improvements, and **Phase 0.6** label capture and label-free validation.
