...
**ISRO hyperspectral (added 2026-08-12, follow-up research pass)**: **HysIS** (operational since 2018) provides 30m resolution across 316 bands (60 VNIR + 256 SWIR), 30km swath, agriculture explicitly named as an application, free/sovereign access via Bhoonidhi — but 30m resolution is *coarser* than the existing Sentinel-2 baseline (10m), reintroducing the mixed-pixel problem this roadmap exists to reduce, and its ~3-month global revisit is incompatible with reacting to a specific claimed event within days. Best positioned as an occasional calibration/reference layer, same conclusion as EnMAP/PRISMA above, not a per-claim evidence source. **EOS-N1 ("Anvesha")** — a newer ISRO hyperspectral satellite explicitly naming agriculture as a civil application — launched 2026-01-12 on PSLV-C62 and **failed to reach orbit** (third-stage roll anomaly during ascent; satellite and 15 co-passenger payloads lost, no announced replacement timeline as of this research pass). It was also DRDO-linked (defence/surveillance dual-use), so civilian commercial access would likely have been restricted even had the launch succeeded. Sources: [HysIS — eoPortal](https://www.eoportal.org/satellite-missions/hysis) · [Bhoonidhi 2026 webinar brief](https://www.nrsc.gov.in/nrscnew/assets/pdf/training_outreach/2026/Bhoonidhi-Webinar-2026.pdf) · [EOS-N1 — Wikipedia](https://en.wikipedia.org/wiki/EOS-N1) · [PSLV-C62 anomaly — ISRO Chairman statement](https://www.newsonair.gov.in/pslv-c62-anomaly-isro-investigates-roll-disturbance-during-third-stage-of-eos-n1-mission-says-chairman-dr-v-narayanan/)
...
### 2.5a Alternatives to hyperspectral (added 2026-08-12, follow-up research pass)

Given §2.3/§2.5's finding that every hyperspectral option researched (EnMAP/PRISMA, HysIS, the now-lost EOS-N1) trades away resolution and/or revisit this module needs, three satellite-based alternatives cover most of what hyperspectral was being considered for, without that penalty:

- **Red-edge multispectral indices (NDRE, Chlorophyll Index Red-Edge, MTCI)** — built on Sentinel-2's existing three red-edge bands (705/740/783nm), already free and already partially represented in `Modeling-Approach.md` §3's Component 2 feature table ("red-edge index"). Documented to detect nitrogen deficiency and stress 1–2 weeks earlier than NDVI, and to resist NDVI's saturation in dense canopy — a direct substitute for hyperspectral's chlorophyll/nitrogen retrieval use case, at Sentinel-2's native 20m/5-day cadence rather than any hyperspectral option's 30m/non-daily-to-3-month cadence. Refinement, not a new data source: naming specific indices and adding a dedicated retrieval step rather than treating "red-edge" as one generic feature.
- **Thermal infrared / canopy temperature — NASA ECOSTRESS**: measures plant canopy temperature (5 thermal bands, 8–12.5µm) as a direct water-stress signal via a different physical principle (evapotranspiration/thermal response) than reflectance spectroscopy entirely — not a hyperspectral substitute so much as a different tool aimed at the same drought/water-stress question. 70m native resolution (38m in-track × 69m cross-track, resampled to 70×70m), 1–5 day revisit depending on latitude (irregular timing — ISS-mounted, not sun-synchronous), free via NASA/AWS Open Data Registry. Notable because it fills a real gap rather than replacing a weak option: no thermal signal exists anywhere in the current Component 2 feature set, and this module's own peril-type list (`data-model.md`) names `drought` and `heatwave` explicitly — exactly the perils canopy temperature is built to evidence.
- **Dense multispectral+SAR time series in place of single-snapshot spectral richness**: academic "multi-sensor spectral synergy" research supports using high-revisit time series (rather than one spectrally-rich pass) to recover much of what hyperspectral's richness offers. This is the design principle already behind choosing Presto — a pixel-*timeseries* transformer — in §4.1/§5, not a new addition.

**Platform caveat (added 2026-08-12, third follow-up pass — resolves an internal inconsistency flagged during review)**: the "how close, quantitatively" figures below (the 10–15-day early-detection lead time, the 83.4% six-level severity classification, the 91% Fusarium-detection accuracy) come from hyperspectral studies whose validated platform is very likely **ground/proximal or UAV-based, not satellite** — one uses a Specim FX-10 benchtop/line-scan camera (a ground instrument), and the other's own framing (per-plant/per-plot severity mapping at the resolution implied) is inconsistent with any satellite-hyperspectral option in §2.3/§2.5, all of which cap out at 30m or coarser. Neither source paper's methods section was confirmed to state "satellite" explicitly, so this is a reasoned inference from instrument/resolution context, not a directly sourced platform claim — flagged as such rather than asserted as fact. **This is the resolution to an apparent tension with §2.6**: §2.6's "no satellite source reliably captures... pre-visible-symptom pest/disease onset" is a claim scoped to satellite platforms specifically, and stands unaffected by ground/UAV research results below — those results describe what hyperspectral sensing *can* do at proximal range, not what any satellite-accessible source (hyperspectral included) reliably does. This module is satellite-only by constitutional mandate (Constitution §4/README), so the ground/UAV figures below describe a capability ceiling this module cannot reach regardless of which satellite-hyperspectral option it picked — they're included for honest completeness about what hyperspectral sensing as a technology can do, not as a claim about what's achievable here.

**Honest limit**: hyperspectral genuinely keeps an edge for species-level pest/disease discrimination (identifying *which* pathogen, not just *that* stress exists) — none of the above close that specific gap, and per the platform caveat above, the ground/UAV research demonstrating that edge doesn't transfer to any satellite option this module could deploy anyway. It isn't a gap this module needs closed: the requirement is damage severity and confidence, not pathogen identification.

**How close, quantitatively (added 2026-08-12, second follow-up pass)**: splits into a sensor-capability question and a platform-access question, and they point different directions.
- *Sensor capability* (red-edge multispectral vs. true hyperspectral, holding platform constant — these red-edge figures ARE satellite-based, Sentinel-2): strong for chlorophyll/nitrogen retrieval — one crop/grass study reports R²=0.94 (chlorophyll) / 0.74 (nitrogen) using Sentinel-2's red-edge chlorophyll index; a mandarin-orchard study validating Sentinel-2-derived red-edge position directly against hyperspectral profiles got adjusted-R²=0.86 (LAI) / 0.80 (chlorophyll); a conifer-forest study validating Sentinel-2A red-edge indices against hyperspectral got r²>0.7 in peak season but only r²=0.6 as the full-season average — closeness is phenology-dependent, strongest at peak growth. For stress-*severity* classification specifically (this module's actual question, via the DSI), one head-to-head device study found a low-cost multispectral sensor's 91.0±3.1% severity-discrimination accuracy matched a hyperspectral method's 91% accuracy on a comparable task — near parity, though platform for this specific comparison is also not confirmed satellite-scale (see platform caveat above). The gap that stays open, **at ground/UAV range only, per the platform caveat above**: earliest pre-visual detection and disambiguating simultaneous co-occurring stressors — one hyperspectral CNN study detected stress 10–15 days earlier than conventional multispectral-index methods.
- *Platform access* (satellite-hyperspectral vs. satellite-multispectral, independent of sensor capability): the alternatives win outright here regardless of the sensor-capability comparison above, since every satellite-hyperspectral option in §2.3/§2.5 (EnMAP/PRISMA 30m/non-daily, HysIS 30m/~3-month, EOS-N1 lost) is worse on resolution and/or revisit than Sentinel-2's 20m/5-day red-edge bands.
- Net: close enough, **at the satellite scale this module actually operates at**, that hyperspectral would not materially change this module's output, because the DSI (`Modeling-Approach.md` §6) already scores relative deviation from a field's own historical baseline rather than absolute biochemical quantification. The early-detection edge hyperspectral shows in ground/UAV research is real as a sensing-technology fact, but not a capability gap this module is forgoing, since no satellite platform (hyperspectral or otherwise) delivers it either — consistent with, not contradicting, §2.6.

Sources: [Sentinel-2 red-edge chlorophyll/nitrogen retrieval — ScienceDirect](https://www.sciencedirect.com/science/article/abs/pii/S0303243412002097) · [Evaluation of Sentinel-2 red-edge bands for LAI/chlorophyll — PMC](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3231680/) · [ECOSTRESS — eoPortal](https://www.eoportal.org/satellite-missions/iss-ecostress) · [NASA ECOSTRESS — Registry of Open Data on AWS](https://registry.opendata.aws/nasa-ecostress/) · [ECOSTRESS 70m resolution — eoPortal instrument page](https://www.eoportal.org/ftp/satellite-missions/iss/ISS-ECOStress_300721/ISS-ECOStress.html) · [Multi-sensor spectral synergies for crop stress — ScienceDirect](https://www.sciencedirect.com/science/article/pii/S003442572200308X) · [Sentinel-2 red-edge vs. hyperspectral, mandarin orchards — ScienceDirect](https://www.sciencedirect.com/science/article/abs/pii/S2352938522000271) · [Sentinel-2A red-edge vs. hyperspectral, conifer forest decline — PMC](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6472624/) · [Multimodal plant stress detection, severity-classification accuracy — Frontiers in Plant Science](https://www.frontiersin.org/journals/plant-science/articles/10.3389/fpls.2025.1670593/full) · [Hyperspectral CNN early stress detection lead time — PMC](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC12484074/)
