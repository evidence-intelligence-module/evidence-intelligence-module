...
## 17. Round 17 — scope boundaries enumerated as Constitution §9, and the roadmap pruned to match

Until now the repo had three stated boundaries (§3 prediction, §4 CCE, §5 intimation channel) and a growing set of unstated ones that were being re-derived per conversation. Two scope decisions made on 2026-08-13 — that the module trains models but does not source training data, and that its output is generic with the consuming service out of scope — turned out to share a shape, and that shape generalises: **the module owns the transformation, not the supply chain at either end.**

Recorded as `constitution.md` §9: one principle, one cross-cutting data-minimisation rule, and 28 boundaries in four groups (upstream supply, downstream consumption, adjacent products, platform). Three of the 28 are pointers to §3/§4/§5 that deliberately do not restate them — Round 4's lesson about duplicated boundary text applied to a section whose whole job is to list boundaries.

Design, alternatives, and the full touch-point list: [`2026-08-13-scope-boundaries-design.md`](2026-08-13-scope-boundaries-design.md) in this directory.

**Four things worth recording here rather than only there:**

1. **All boundaries are permanent, deliberately.** A two-tier scheme mirroring §3's "out of scope, for now" against §4's flatter prohibition was considered and rejected. The cost is accepted knowingly: supporting another country's scheme now needs a §8 amendment. The argument was that §8 asks only for a written decision and a reason, which anyone should produce anyway, and that a uniform rule erodes less easily than a graduated one.

2. **"No personal data" was drafted, found false, and replaced.** `002`'s own personal-data query had already established that geometry plus event date plus peril type identifies a specific farm — so the module processes personal data today, inherently. A boundary asserting its absence would have been false on the module's primary input. §9.1 is therefore data *minimisation*: the surface is exactly as wide as the purpose requires and never grows. This closed the attachment half of that query and explicitly did **not** close the DPDP-versus-retention half, which survives as a `documents/` change.

3. **Five prunes, 35 touch points, no code deleted** — none of it had been built. Struck: commercial satellite procurement (`T037`, `FR-019`, the `access_model` column, the disabled client stub), field-boundary assistance (`T038`, `FR-014`), claim-outcome capture and label export (`TV-01`/`TV-01a`/`TV-02`, `FR-024`, the `claim_outcomes` table and endpoint), the ablation harness (`TV-05`, justified by label scarcity that no longer applies), and supplementary-evidence attachments (`FR-006`, `T014`/`T018`/`T019`, the table and endpoint). Struck requirement identifiers were retained with a removal note rather than renumbered, since `tasks.md`, `plan.md`, `quickstart.md` and the checklists cite them by number.

4. **A disabled stub is not a neutral thing to leave behind.** `commercial_tasking_client.py` had been retained specifically so a future budget decision would need only a config change. That is the right call under a deferral and the wrong one under a boundary — it leaves a mechanism in place for crossing a line that now requires an amendment. Removed rather than left disabled.

Two consequences are recorded rather than hidden. The causation low-confidence threshold and the confidence-tier values both wanted claim-outcome data; with `TV-01` struck, neither has an in-module source, so either the supplier contract carries outcomes or both stay permanently unset — survivable under the existing "unset rather than invented" discipline. And with sourcing external, a supplier-declared `label_provenance` string on the saved model artifact is the only thing the repo can honestly record about where its training data came from.

Left alone deliberately: every entry above this one, including their references to now-struck task IDs — same historical-accuracy treatment as Rounds 12, 15 and 16.
